...
object MenWomenTest{
  ...
  /** Process for a man. */
  def man(me: Int, mws: MenWomenServer) = proc{
    partnerOfMan(me) = mws.manSync(me.toString).toInt
  }

  /** Process for a woman. */
  def woman(me: Int, mws: MenWomenServer) = proc{
    partnerOfWoman(me) = mws.womanSync(me.toString).toInt
  }

  /** Do a single test. */
  def doTest = {
    val n = Random.nextInt(20) // # men, women
    partnerOfMan = new Array[Int](n); partnerOfWoman = new Array[Int](n)
    val mwt = new MenWomenServer
    val men = || (for(i <- 0 until n) yield man(i, mwt))
    val women = || (for(i <- 0 until n) yield woman(i, mwt))
    run(men || women)
    mwt.shutdown
    for(m <- 0 until n)
  ...
  def main(args: Array[String]) = {
    for(i <- 0 until 100000){ doTest; if(i%200 == 0) print(".") }
    println; exit
  }
}
\end{scala}
