...
\SCALA{(i,j)}; you may assume a definition for \SCALA{neighbours} has been
provided.
%
%% \begin{scala}
%%   def neighbours(i:Int, j:Int) : List[Coord] = { 
%%     var ns = Nil : List[Coord];
...
process should work on a strip of \SCALA{height = N/p} rows.  (Each process
may use an \SCALA{N} by \SCALA{N} array to store the labels, but will only
...
the \SCALA{myDone} variables the workers for higher strips; thus this final
\SCALA{done} value is accumulated in this boolean;

\item
Then, in order, starting from the worker for the bottom strip, the top rows
are passed up, together with the final  \SCALA{done} value.
\end{itemize}
%
